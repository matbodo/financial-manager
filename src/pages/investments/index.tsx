...
export function InvestimentosPage() {
    ...
    const [InvestmentStorage, setInvestmentStorage] = useState<Array<{ name: string; value: string }>>([]);
    ...
    const handleSaveInvestment = () => {
        if (name === '' || price === '' || parseFloat(price) <= 0) {
            alert('Por favor, preencha todos os campos corretamente antes de adicionar uma investmento.');
            return;
        } else {
            setInvestmentStorage([...InvestmentStorage, { name: name, value: price }])
            setOpenForm(true);
        }
    };
    ...
    const handleRemove = (indexToRemove) => {
        const uptadeInvestment = InvestmentStorage.filter((_, index) => index !== indexToRemove);
        setInvestmentStorage(uptadeInvestment);
    };
    ...
    useEffect(() => {
        window.localStorage.setItem('InvestmentsList', JSON.stringify(InvestmentStorage));
    }, [InvestmentStorage]);
